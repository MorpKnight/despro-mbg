...
export default function SekolahDashboard() {
  ...
  useEffect(() => {
    if (user?.role === "admin_sekolah") setSelectedSchool(schools[0].id);
  }, [user?.role, schools]);
  ...
  const current = schoolData[selectedSchool as keyof typeof schoolData] || defaultSchoolData;
              ...
              <View className="space-y-2">
                <View className="flex-row justify-between">
                  ...
                  <View className="flex-row items-center">
                    <View
                      className={`w-2 h-2 rounded-full ${current.attendance.deviceStatus === "Online"
                        ? "bg-emerald-500"
                        : "bg-gray-400"
                        } mr-1`}
                    />
            ...
            <View className="flex-1 bg-white rounded-2xl p-6 shadow-sm mb-3">
              ...
              {current.announcements.map((a) => (
                <View key={a.id} className="flex-row mb-4">
                  <View
                    className={`w-7 h-7 rounded-full border ${a.color === "blue"
